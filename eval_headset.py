...
if __name__ == '__main__':
    ...
    speech_path = '/home/ubuntu/Data/DFN/textfiles/DNS5_val_headset.txt'
    ...
    for model_name in model_names: #for each model (or set of training RIRs)
        ...
        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S")+' || Running evaluation of '+model_name+' evaluated on Headset.')
        for noisy, meta in tqdm.tqdm(dataloader):
            ...
            for i in range(batch_size):
                ...
                df.loc[didx+i, 'eval_rirs'] = 'none_headset'
        ...
        df.to_csv(pjoin(results_path, model_name+'_evaluatedOn_Headset.csv'), index=False)
